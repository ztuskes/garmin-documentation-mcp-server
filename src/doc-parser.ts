...
export class LocalDocumentationParser {
  ...
  private async parseClassList() {
    const classListPath = path.join(this.docsPath, 'class_list.html');
    if (!fs.existsSync(classListPath)) return;
  ...
  private async parseToyboxModules() {
    const toyboxPath = path.join(this.docsPath, 'Toybox');
    if (!fs.existsSync(toyboxPath)) return;
  ...
  private async parseModule(filePath: string, moduleName: string): Promise<void> {
  ...
  private async parseModuleDirectory(dirPath: string, moduleName: string): Promise<void> {
  ...
  private async parseClass(filePath: string, className: string, moduleName: string): Promise<void> {
    ...
    // Extract methods
    const methods: ParsedMethod[] = [];
    const constructors: ParsedMethod[] = [];
  ...
  private parseMethodDetail($: cheerio.CheerioAPI, $detail: cheerio.Cheerio<any>): ParsedMethod | null {
  ...
  private parsePropertyDetail($: cheerio.CheerioAPI, $detail: cheerio.Cheerio<any>): ParsedProperty | null {
    const signature = $detail.find('.signature').first();
    if (!signature.length) return null;
  ...
  private async parseMethodsFromPage($: cheerio.CheerioAPI, methods: ParsedMethod[]): Promise<void> {
